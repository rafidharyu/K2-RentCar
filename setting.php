...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate form input to ensure no empty values are submitted
    if (empty($username) || empty($email) || empty($fullname) || empty($phone_number)) {
        $error_message = "<div><p>Error: All fields are required.</p></div><br>";
    } else {
        ...
        if ($result_check_username->num_rows > 0) {
            ...
            $error_message .= "<div><p>Error: Username already exists.</p></div><br>";
        }

        // Check if email already exists
        $sql_check_email = "SELECT * FROM users WHERE email = ? AND id <> ?";
        ...
        if ($result_check_email->num_rows > 0) {
            ...
            $error_message .= "<div><p>Error: Email already exists.</p></div><br>";
        }

        // Check if phone number already exists
        $sql_check_phone = "SELECT * FROM users WHERE phone_number = ? AND id <> ?";
        $stmt_check_phone = $conn->prepare($sql_check_phone);
        $stmt_check_phone->bind_param("si", $phone_number, $user_id);
        ...
        if ($result_check_phone->num_rows > 0) {
            ...
            $error_message .= "<div><p>Error: Phone number already exists.</p></div><br>";
        }

        // If no errors, update the user record
        if (empty($error_message)) {
            $sql_update = "UPDATE users SET username = ?, email = ?, fullname = ?, phone_number = ? WHERE id = ?";
            $stmt_update = $conn->prepare($sql_update);
            $stmt_update->bind_param("ssssi", $username, $email, $fullname, $phone_number, $user_id);

            if ($stmt_update->execute()) {
                $_SESSION['username'] = $username; 
                $success_message = "<div><p>Record updated successfully.</p></div><br>";
            } else {
                $error_message = "<div><p>Error updating record: " . $conn->error . "</p></div><br>";
            }
            $stmt_update->close();
        }
...
<head>
	...
	<title>K2 RentCar - Setting</title>
...
<div class="fixed mt-32 pt-3 w-64 flex h-full" id="sidebar">
    <div class="bg-merah overflow-y-auto flex flex-col flex-grow pt-3">
        <nav class="space-y-3 px-4 mt-5">
            <a href="dashboard.php" class="text-gray-200 flex items-center space-x-3 p-2 rounded-lg transition duration-300 transform hover:bg-[#142445] hover:bg-opacity-20 hover:scale-105 hover:shadow-lg">
                <div>
            ...
            <div class="flex-row w-full ps-14">
                <div class="relative bg-dongker rounded-bl-full flex">
                    ...
                    <div class="w-1/4 me-10 flex">
                        ...
                        <a href="login.html" target="_blank" class="text-sm font-medium text-white bg-merah px-10 py-5"><span class="text-merah">____</span>
                        <?php
		...
		<main class="absolute start-80 top-44 pr-20">
			<div class="head-title flex justify-between items-center">
				<div class="flex flex-row">
                    <div class="pt-1 me-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 48 48"><defs><mask id="ipSSettingOne0"><g fill="none" stroke-linejoin="round" stroke-width="4"><path fill="#fff" stroke="#fff" d="m34 41l10-17L34 7H14L4 24l10 17z"/><path fill="#000" stroke="#000" d="M24 29a5 5 0 1 0 0-10a5 5 0 0 0 0 10Z"/></g></mask></defs><path fill="#142445" d="M0 0h48v48H0z" mask="url(#ipSSettingOne0)"/></svg>
                    </div>
					<h1 class="text-2xl font-bold mb-5 text-dongker">Setting</h1>
				</div>
			</div>
            
            <div class="mt-2">
                <div class="rounded-3xl shadow-lg bg-white py-10 px-16">
                    <div class="flex flex-row">
                        <div class="pt-1 me-2">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="black" d="M10 4a4 4 0 0 0-4 4a4 4 0 0 0 4 4a4 4 0 0 0 4-4a4 4 0 0 0-4-4m7 8a.26.26 0 0 0-.26.21l-.19 1.32c-.3.13-.59.29-.85.47l-1.24-.5c-.11 0-.24 0-.31.13l-1 1.73c-.06.11-.04.24.06.32l1.06.82a4.2 4.2 0 0 0 0 1l-1.06.82a.26.26 0 0 0-.06.32l1 1.73c.06.13.19.13.31.13l1.24-.5c.26.18.54.35.85.47l.19 1.32c.02.12.12.21.26.21h2c.11 0 .22-.09.24-.21l.19-1.32c.3-.13.57-.29.84-.47l1.23.5c.13 0 .26 0 .33-.13l1-1.73a.26.26 0 0 0-.06-.32l-1.07-.82c.02-.17.04-.33.04-.5s-.01-.33-.04-.5l1.06-.82a.26.26 0 0 0 .06-.32l-1-1.73c-.06-.13-.19-.13-.32-.13l-1.23.5c-.27-.18-.54-.35-.85-.47l-.19-1.32A.236.236 0 0 0 19 12zm-7 2c-4.42 0-8 1.79-8 4v2h9.68a7 7 0 0 1-.68-3a7 7 0 0 1 .64-2.91c-.53-.06-1.08-.09-1.64-.09m8 1.5c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5c-.84 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5"/></svg>
                        </div>
                        <h1 class="text-xl font-semibold mb-5 text-black">Account Setting</h1>
                        </div>
                        <div class="bg-white mb-7 py-4 pl-6 pr-96 rounded-3xl border-2 border-dongker">
                            <span>
                                <h3 class="mb-10 text-xl font-medium">General Info</h3>
                            </span>
                            <?php 
                                if (!empty($error_message)) {
                                    echo $error_message;
                                    echo "<div><br>
                                        <button class='bg-white border-2 border-dongker hover:bg-dongker text-current text-md font-bold py-2 px-6 rounded-lg transition duration-300'>
                                            <a href='to-setting.php'>Confirm</a>
                                        </button></div><br>";
                                }
                            ...
                            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" class="space-y-6">
                                ...
                                <div class="flex">
                                    <span class="items-center me-40">
                                        <p>Full Name:</p>
                                        <input type="text"
                                            name="fullname"
                                            id="fullname"
                                            class="border border-gray-400 rounded-lg px-4 py-2 w-full focus:outline-none focus:border-merah focus:ring-1 focus:ring-merah"
                                ...
                                <div class="flex">
                                    <span class="absolute flex items-center right-48 bottom-28">
                                        <div class="pt-1 me-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="none" stroke="#ff0000" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 3H7a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h8m4-9l-4-4m4 4l-4 4m4-4H9"/></svg>
                                        </div>
                                        <a href="logout.php" class="hover:text-merah">Log Out</a>
                                    </span>
                                </div>
                                <div class="flex justify-left items-center mt-8 mb-1">
                                    <button type="submit"
                                            class="bg-white border-2 border-dongker hover:bg-dongker text-current text-md font-bold py-2 px-6 rounded-lg transition duration-300">
                                        Save
                                    </button>
                                </div>
                            </form>
